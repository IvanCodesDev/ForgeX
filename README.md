...
└─ /api/v1/gcode/analyze ───────────────┐
                              ...
                              ├─ 文件仓库健康探针与可校验备份/恢复
                              ├─ PostgreSQL v1/v2/v3/v4/v5/v6 迁移契约与校准/数据源/知识库/分享/任务运行时（可选）
                              ├─ 确定性摘要、单位与稳定错误码
...
- **存储**：默认 `PERSISTENCE_PROVIDER=file` 写入 `data/`，容器部署可挂载持久化卷；文件作业仓库提供逐条 SHA-256 的版本化备份、全量预检恢复和就绪探针。设置 `PERSISTENCE_PROVIDER=postgres` 与 `POSTGRES_URL` 后，校准治理、数据源、知识库、分享快照和 Node 分析任务历史会使用 PostgreSQL v2–v6 表、事务和 RLS；分享公开读取保留 token 入口并记录访问审计，任务事件与终态报告跨重启恢复，重启时中断中的任务会如实标为失败。`backend/database/postgresql/` 的迁移必须先应用并通过 `npm run postgres:migrations:check`。
