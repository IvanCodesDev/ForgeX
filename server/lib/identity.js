/* 统一请求身份与资源级授权。
   ...
   Stage 8.2：resolveIdentity 为 async——AUTH_AUTHORITY=csharp 时会话身份由
   ForgeX.Api 持有，partnerSSO.identity(req) 是远程反查（见 partner-sso-proxy.js）；
   默认 node 模式仍是同步内存查找，await 结果不变。 */
"use strict";
...
async function resolveIdentity(req, rc, ctx) {
  const partner = ctx.partnerSSO && ctx.partnerSSO.enabled ? await ctx.partnerSSO.identity(req) : null;
  if (partner) {
