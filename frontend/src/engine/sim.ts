/* FORGE·X — 打印仿真核心：状态机 / 运动执行 / 温控惯性 / 耗材消耗 / 质量推导 / 故障注入。
   自 js/sim.js 机械迁移：物理与状态机逻辑逐行保留，依赖改为直接 import 已迁 TS 模块。 */
import { ThermalSim, clamp, fmtHuman, lerp, mulberry32, valueNoise2D } from "./util.ts";
import * as FXMachineProfile from "./machine-profile.ts";
import { slice as slicerSlice } from "./slicer.ts";
import { materials as profileMaterials } from "./profile-registry.ts";
...
const FALLBACK_MATERIALS: Record<string, FallbackMaterial> = {
  PLA: {
    id: "PLA",
    name: "PLA",
    nozzleTemp: 210,
...
/** 规范化材料（profile-registry 的 normalizeMaterial 保证便捷字段运行时齐全）。 */
...
/** 与 js/sim.js 顶层求值同语义：profile-registry 的材料库（含社区注册）优先。 */
const MATERIALS: Record<string, SimMaterial> =
  (profileMaterials as unknown as Record<string, SimMaterial>) || FALLBACK_MATERIALS;
...
/** 切片结果（自产切片与导入 G-code 同构；导入侧带 claims 等扩展字段）。 */
export interface SimSlice {
  layers: Array<{ z: number; paths: SlicePath[]; extLen: number; travelLen: number; timeSec: number }>;
  totalLayers: number;
  height: number;
  stats: {
    extLenMm: number;
  ...
  claims?: ParsedGcodeResult["claims"];
  [key: string]: unknown;
}
...
export interface SimTelemetry {
  ...
  retractions: number;
  retractedMm: number;
  stringingRiskWeighted: number;
  stringingTravelMm: number;
  retractionStress: number;
  machineId: string;
  machineProfile: FXMachineProfile.MachineProfile;
  warpRisk?: number;
  overhangRisk?: number;
}
...
interface RunPath extends SlicePath {
  filamentMm?: number;
  _cum?: number[];
}
...
interface LayerRun {
  idx: number;
  data: SimSlice["layers"][number];
  paths: RunPath[];
  pathIdx: number;
  phase: "travel" | "retract" | "unretract" | "extrude" | "layerEnd";
  travel: {
...
export class FXSim {
  ...
  model: SimModel | null;
  slice: SimSlice | null;
  importedToolpath: { name: string; sourceText: string; parsed: ParsedGcodeResult } | null;
  ...
  bedTargetY: number;
  levelMesh: LevelMesh | null;
  leveledOnce: boolean;
  faultInfo: FaultInfo | null;
  lastQuality: QualityReport | null;
  _telemetry: SimTelemetry | null;
  nozzleNow = 0;
  bedNow = 0;

  private _slicedSpeed: number;
  private _bedErr: ((x: number, y: number) => number) | null;
  private _bedErrId: string;
  private _bedErrSeed?: number;
  private _probeRnd: boolean | null = null;
  private _probeRndId?: string;
  private _probeNoiseFn?: (x: number, y: number) => number;
  private _profCache: FXMachineProfile.MachineProfile | null = null;
  private _profOverride: Partial<FXMachineProfile.MachineProfile> | null = null;
  private _layer: LayerRun | null;
  private _level?: LevelRun;
  private _layerBaseY = 0;
  private _doneExtBefore: number;
  private _doneTimeBefore: number;
  private _milestone: number;
  private _lowMatWarned: number;
  private _heatMinTimer = 0;
  private _heatElapsed = 0;
  private _resumeState?: SimState;
  private _clogT = 0;
  private _slipT = 0;
  private _thermalT = 0;
  private _lastNozzleNow: number | null = null;
  private _nozzleRate = 0;

  static MATERIALS = MATERIALS;
  static COLORS = COLORS;

  constructor(fx: FXScene, bus: BusLike) {
    this.fx = fx; // FXScene
    ...
    this.nozzleT = new ThermalSim(26, 11, 3.6, 0);
    this.bedT = new ThermalSim(26, 30, 1.0, 4.13);
  ...
  log(lv: string, msg: string): void {
    this.bus.emit("log", { lv, msg });
  }
  ...
  get material(): SimMaterial {
    return MATERIALS[this.settings.material] || MATERIALS.PLA || FALLBACK_MATERIALS.PLA!;
  }
  get partColor(): number {
    return COLORS[this.settings.colorIdx]!.hex;
  }

  /* ── 机台身份与固有物理特征 ──────────────
     故障不是抽出来的：每台机器有确定性的磨损/加热器健康度/环境参数，
     仿真过程按这些特征真实演化，故障是因果链的结果。见 machine-profile。 */
  ...
  get machineProfile(): FXMachineProfile.MachineProfile {
  ...
  setMachineInstance(
    n: number,
    overrides?: Partial<FXMachineProfile.MachineProfile> | null
  ): FXMachineProfile.MachineProfile {
  ...
  setModel(model: BuiltModel | SimModel, resetTf?: boolean): void {
    this.importedToolpath = null;
    this.model = model as SimModel;
  ...
  reslice(): void {
    ...
    this.slice = slicerSlice(this.model as unknown as BuiltModel, this.tf, this.settings as never) as SimSlice;
    this._slicedSpeed = this.settings.speed;
    this.printer.attachPart(this.model as unknown as BuiltModel, this.tf, this.partColor, this.settings.layerHeight);
    this.printer.showGhost(this.state === "idle" || this.state === "done");
    this.bus.emit("sliced", { ms: performance.now() - t0 });
  }

  updateTf(patch: Partial<SimTf>): void {
    ...
    this.tf.offX = clamp(this.tf.offX, -lim, lim);
    this.tf.offY = clamp(this.tf.offY, -lim, lim);
    this.printer.updatePartTf(this.tf);
  }

  updateSettings(patchIn: Partial<Record<string, number | string | boolean>>): void {
    const patch = Object.assign({}, patchIn);
    const geomKeys = [
      "layerHeight",
      "perimeters",
      "solidLayers",
    ...
    if (busy) {
      // 温度/风扇/速度实时生效
      if (patch.nozzleTemp != null) {
        this.nozzleT.setTarget(patch.nozzleTemp as number);
        this.log("info", `喷嘴目标温度调整 → ${patch.nozzleTemp}°C`);
      }
      if (patch.bedTemp != null) {
        this.bedT.setTarget(patch.bedTemp as number);
        this.log("info", `热床目标温度调整 → ${patch.bedTemp}°C`);
      }
  ...
  setPrinterModel(id: string): boolean {
    if (!["idle", "done"].includes(this.state)) {
      this.log("warn", "打印进行中，不可切换机型");
      return false;
    }
    ...
    if (this.importedToolpath && this.slice) {
      p.attachToolpath(this.slice as never, this.partColor);
      p.showGhost(true);
    } else if (this.model) {
      this.reslice();
    }
  ...
  loadImportedToolpath(
    ...
    this.tf = { scale: 1, rotZ: 0, offX: 0, offY: 0 };
    this.slice = parsed as unknown as SimSlice;
    ...
    this.log(
      "ok",
      `真实 G-code 已载入：${this.model.name} · ${parsed.totalLayers} 层 · ${parsed.stats.filamentM.toFixed(2)} m 耗材`
    );
    return this.model;
  }
  ...
  start(): void {
    ...
    if (this.state === "done" || this.state === "idle") {
      ...
      this.log(
        "info",
        `任务开始：${this.model!.name} · ${this.slice.totalLayers} 层 · 预计 ${fmtHuman(this.estimateTotal())}`
      );
  ...
  resume(): void {
    ...
    if (this.faultInfo) {
      ...
      if (this.nozzleT.heaterBroken) {
        this.nozzleT.heaterBroken = false;
        this.log("info", "加热器供电已恢复，重新升温");
      }
    }
    // 恢复温度目标（热失控保护会切断加热）
    this.nozzleT.setTarget(this.settings.nozzleTemp);
  ...
  private _raiseFault(F: FaultInfo): void {
  ...
  /** 当前挤出状态快照，喂给 machine-profile 的机理模型 */
  private _mechState(): FXMachineProfile.HotendState & FXMachineProfile.FeedState {
    ...
    return {
      ...
      materialFlowMm3s:
        (this.material as { flowMm3s?: number }).flowMm3s || FXMachineProfile.FLOW_MM3S[st.material] || 9,
      flowMm3s: v * st.extrusionWidth * st.layerHeight,
      extrudedMm3: ((this.usedG - (T ? T.usedG0 : 0)) * 1000) / mat.densityG,
      spoolRemainFrac: clamp((this.spoolTotalG - this.usedG) / this.spoolTotalG, 0, 1),
    };
  }
   ...
   */
  private _mechWatch(dt: number): void {
    if (this.state !== "print" || this.faultInfo) {
      this._clogT = 0;
      this._slipT = 0;
      return;
    }
    ...
    if (this._clogT >= MECH_HOLD_S) {
      ...
      this._raiseFault({
        name: "喷嘴堵塞预警",
        msg:
          `挤出机负载 ${(clog * 100).toFixed(0)}% 于阈值，持续 ${MECH_HOLD_S}s` +
    ...
    if (this._slipT >= MECH_HOLD_S) {
      ...
      this._raiseFault({
        name: "断料检测触发",
        msg:
          `送料齿轮打滑（咬合力 ${(prof.feederGrip * 100).toFixed(0)}%，料架阻力 ${(prof.spoolDrag * 100).toFixed(0)}%，` +
  ...
  private _thermalWatch(dt: number): void {
    const mdt = dt * this.simMult;
  ...
  /** 机台固有床面误差场（按机台编号确定性生成 → 同一台机器重复调平结果一致，
      不同实例的床面各不相同，这是虚拟机群里机台差异的来源之一） */
  private _bedErrField(): (x: number, y: number) => number {
    const id = this.machineId;
    ...
    const half = Math.max(25, (this.printer.BED_SIZE || 256) / 2);
    this._bedErr = (x: number, y: number) =>
      tiltX * x + tiltY * y + (noise((x / half) * 1.5 + 3.7, (y / half) * 1.5 + 8.2) - 0.5) * 0.18;
    this._bedErrId = id;
  ...
  bedErrorAt(x: number, y: number): number {
    return this._bedErrField()(x, y);
  }

  /** 探针重复性误差（±3μm 量级）。按机台 + 探测点确定性生成：
      同一台机器、同一个点，每次探测得到同样的读数——真实探针的重复精度就是这样，
      而且这保证了整条调平数据链可复现。 */
  private _probeNoise(x: number, y: number): number {
    if (!this._probeRnd || this._probeRndId !== this.machineId) {
      this._probeRndId = this.machineId;
      this._probeNoiseFn = valueNoise2D((this._bedErrSeed || 1) ^ 0x2545f491);
      this._probeRnd = true;
    }
  ...
  meshCompAt(x: number, y: number): number {
    ...
    const step = half / 2;
    const u = clamp((x + half) / step, 0, 4);
    const v = clamp((half - y) / step, 0, 4);
    const i0 = Math.min(3, Math.floor(u)),
      j0 = Math.min(3, Math.floor(v));
    const fx = u - i0,
      fy = v - j0;
    return lerp(lerp(g[j0]![i0]!, g[j0]![i0 + 1]!, fx), lerp(g[j0 + 1]![i0]!, g[j0 + 1]![i0 + 1]!, fx), fy);
  }

  runLeveling(): void {
    if (this.state !== "idle" && this.state !== "done") {
      this.log("warn", "仅待机状态可执行手动调平");
      return;
    }
    if (this.state === "done") {
      // 清除上一件成品再校准
  ...
  private _finishLeveling(): void {
    ...
    const probeAt = (x: number, y: number) => {
      const u = clamp((x + probeR) / probeR, 0, 2); // 0..2 列
      const v = clamp((probeR - y) / probeR, 0, 2); // 0..2 行
      const i0 = Math.min(1, Math.floor(u)),
        j0 = Math.min(1, Math.floor(v));
      const fx = u - i0,
        fy = v - j0;
      return lerp(
        lerp(S[j0 * 3 + i0]!, S[j0 * 3 + i0 + 1]!, fx),
        lerp(S[(j0 + 1) * 3 + i0]!, S[(j0 + 1) * 3 + i0 + 1]!, fx),
        fy
      );
    };
    const mesh: number[][] = [];
  ...
  tick(dt: number, _t?: number): void {
    // _t：场景传入的累计时间，仿真核不需要（保留形参以对齐 onTick 签名）
    void _t;
    ...
    this.bedNow = bActual;
    P.nozzleHotFrac = clamp((nActual - 40) / 220, 0, 1);
    P.bedHotFrac = clamp((bActual - 35) / 80, 0, 1);
    ...
    P.extrudeRate =
      this.state === "print" && this._layer && this._layer.phase === "extrude" ? this._layer.curSpeed || 0 : 0;
    P.setSpoolFrac((this.spoolTotalG - this.usedG) / this.spoolTotalG);
  }

  private _tickHeat(dt: number): void {
    const mdt = dt * this.simMult;
    this._heatMinTimer -= mdt;
    this._heatElapsed = (this._heatElapsed || 0) + mdt;
    // 归位动作
    this.headPos.x = lerp(this.headPos.x, -110, 1 - Math.exp(-dt * 2));
    this.headPos.y = lerp(this.headPos.y, 110, 1 - Math.exp(-dt * 2));
    ...
    if (!this.faultInfo && this._heatElapsed > HEAT_TIMEOUT_S && !this.nozzleT.reached(3)) {
      ...
      this._raiseFault({
        name: "加热失败",
        msg:
          `预热 ${Math.round(this._heatElapsed)}s 后喷嘴仍停在 ${this.nozzleNow.toFixed(0)}°C，` +
  ...
  private _tickLevel(dt: number): void {
    const L = this._level!;
    const mdt = dt * this.simMult;
    if (L.idx < 0) {
      L.idx = 0;
      L.phase = "move";
      L.t = 0;
      this.bedTargetY = this.printer.NOZZLE_Y - 10;
    }
    const target = L.pts[L.idx]!;
    if (L.phase === "move") {
      const dx = target.x - this.headPos.x,
        dy = target.y - this.headPos.y;
      const d = Math.hypot(dx, dy);
      const step = 190 * mdt;
      if (d <= step) {
        this.headPos.x = target.x;
        this.headPos.y = target.y;
        L.phase = "probe";
        L.t = 0;
      } else {
      ...
      if (L.t >= 0.95) {
        ...
        this.log(
          "info",
          `探测点 ${L.idx + 1}/9（X${target.x} Y${target.y}）：Z = ${z >= 0 ? "+" : ""}${z.toFixed(3)} mm`
        );
        L.idx++;
  ...
  private _zCompAt(x: number, y: number, z: number): number {
    const FADE_H = 6;
    if (!this.levelMesh || z >= FADE_H) return 0;
    return this.meshCompAt(x, y) * (1 - z / FADE_H);
  }

  private _startLayer(idx: number): void {
    ...
    const colors = {
      perimeter: this.partColor,
      solid: this.partColor,
      infill: this._dimColor(this.partColor, 0.82),
      skirt: this._dimColor(this.partColor, 0.6),
      support: 0x7d86a8,
    };
    const usable = layer.paths.filter((p) => p.pts.length >= 2 && (p.len ?? 0) > 0.4);
  ...
  private _dimColor(hex: number, f: number): number {
  ...
  private _setupTravelTo(pathIdx: number): void {
    const L = this._layer!;
    if (pathIdx >= L.paths.length) {
      L.phase = "layerEnd";
      return;
    }
  ...
  private _tickPrint(dt: number): void {
    const L = this._layer;
    if (!L) return;
    ...
    while (budget > 1e-6 && guard-- > 0) {
      if (L.phase === "retract") {
        ...
        this.headPos.x = lerp(L.travel!.from.x, L.travel!.to.x, t);
        this.headPos.y = lerp(L.travel!.from.y, L.travel!.to.y, t);
    ...
    this.progress = clamp(doneT / this.slice!.stats.timeSec, 0, 1);
    const pct = Math.floor(this.progress * 10) * 10;
    if (pct >= this._milestone + 10 && pct < 100) {
      this._milestone = pct;
      this.log("info", `打印进度 ${pct}% · 剩余约 ${fmtHuman(this.estimateRemaining())}`);
    }
    // 耗材预警
    const remainG = this.spoolTotalG - this.usedG;
    if (remainG < 150 && this._lowMatWarned < 1) {
      this._lowMatWarned = 1;
      this.log("warn", "耗材余量低于 15%，请准备备用料盘");
    }
    if (remainG <= 0.5 && !this.faultInfo) this.injectFault("runout");
  }

  private _pointAt(path: RunPath, dist: number): { x: number; y: number } {
    if (!path._cum) {
      path._cum = [0];
  ...
  private _consumeFilament(extMm: number, path?: RunPath): void {
    let filamentMm: number, mm3: number;
    if (this.importedToolpath && path && (path.filamentMm ?? 0) > 0 && (path.len ?? 0) > 0) {
  ...
  private _finishPrint(): void {
    ...
    this.log(
      "ok",
      `打印完成：${this.model!.name} · 用时 ${fmtHuman(this.machineElapsed)}（机时）· 耗材 ${(this.usedG - (this._telemetry ? this._telemetry.usedG0 : 0)).toFixed(1)} g`
    );
    if (this._telemetry) {
      ...
      this.log(
        "info",
        `成品实测质量评级 ${this.lastQuality.grade}（综合 ${this.lastQuality.score} 分）· 详见「质量」面板`
      );
    }
    ...
    if (scrap) {
      ...
      this.bus.emit("job-record", { status: "fail", fault: scrap.reason, scrap });
    } else {
   ...
   */
  private _evaluateScrap(): { reason: string; risk: number; msg: string } | null {
    const T = this._telemetry;
    ...
    const warp = FXMachineProfile.warpRisk(prof, {
      ...
      shrinkage:
        (mat as { shrinkage?: number }).shrinkage != null
          ? (mat as { shrinkage?: number }).shrinkage!
          : FXMachineProfile.SHRINKAGE[st.material] || 0.4,
    ...
    if (warp >= 1) {
      return {
        ...
        risk: T.warpRisk,
        msg:
          `床温 ${T.settings0.bedTemp}°C（${st.material} 下限 ${mat.bedMin}°C）· ` +
  ...
  static computeQuality(st: SimSettings, model?: SimModel | null): QualityCheck[] {
    const m = MATERIALS[st.material]!;
    const out: QualityCheck[] = [];
    const push = (name: string, score: number, tip: string) => {
    ...
    // 层间结合强度
    let s = 100;
    const [tLo, tHi] = m.nozzleRange as [number, number];
    ...
    push(
      ...
      s,
      st.nozzleTemp < tLo
        ? `喷嘴温度低于 ${m.name} 推荐区间（${tLo}–${tHi}°C），层间熔合不足`
        : st.nozzleTemp > tHi
          ? "温度偏高，边缘易塌陷、拉丝增多"
          : `温度处于 ${m.name} 理想窗口（理想值约 ${ideal}°C）`
    );
    ...
    push(
      ...
      s,
      st.bedTemp < m.bedMin
        ? `热床低于 ${m.name} 最低要求 ${m.bedMin}°C，存在翘边风险`
        : "热床温度满足附着要求，封闭腔体进一步降低翘边概率"
    );
    ...
    push(
      "尺寸精度预估",
      s,
      st.material === "ABS" ? "ABS 收缩率较高（约 0.6%），关键配合面建议预留补偿" : "冷却均匀，收缩可控"
    );
  ...
  static computeActualQuality(
    tel: SimTelemetry,
    st: SimSettings,
    model: SimModel | null,
    run?: { elapsed: number; usedG: number } | null
  ): QualityReport {
    void st;
    void model;
    ...
    if (tel.leveled) {
      ...
      push(
        "首层成形与调平",
        s,
        `已按 9 点实测网格补偿（床面最大偏差 ${(tel.levelMax ?? 0) >= 0 ? "+" : ""}${(tel.levelMax || 0).toFixed(3)} mm）` +
    ...
    const stringingRisk = tel.stringingTravelMm > 0 ? tel.stringingRiskWeighted / tel.stringingTravelMm : 0;
