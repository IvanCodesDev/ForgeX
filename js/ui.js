  ...
  class FXUI {
    ...
    _aboutModal() {
      ...
      mask.innerHTML = `<div class="modal"><h3><span class="ph-tick"></span>FORGE·X 智造洞察</h3>
        <p>FORGE·X 工业 3D 打印仿真 × 智造数据分析：左手模拟四款可切换机型（CoreXY 封闭式 / i3 龙门 / Delta 并联臂 / 工业大幅面龙门）的完整打印流程，右手对生产数据做自然语言分析。</p>
        <p><b>快速上手</b><br>
        ...
        ⑤ 顶部「<b>洞察</b>」：对生产数据自然语言提问 — 故障归因、材料对比、成本趋势，结论可一键定位回 3D 视口</p>
        <p><b>数据来源</b>　内置示例数据开箱即用；支持上传自己的 CSV；每次模拟打印完成也会自动采集为「模拟采集」数据集，形成自洽闭环。</p>
