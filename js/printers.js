...
(function (root) {
  ...
  /* ══════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════ */
  class FXPrinterI3 extends FXPrinterBase {
    _buildMachine() {
      ...
      for (const sx of [-1, 1]) for (const sz of [-1, 1])
        this._cyl(14, 16, 9, M.blackPlastic, sx * 175, -3, sz * 185 + 10);
      // Y 向中央型材 + 双导轨
      ...
      // 四角调平旋钮
      for (const sx of [-1, 1]) for (const sz of [-1, 1])
        this._cyl(9, 9, 6, M.blackPlastic, sx * 122, -16, sz * 122, g);
      ...
      for (let i = 0; i < 6; i++)
        this._box(30, 1.8, 18, M.aluBright, 0, 18 + i * 3, 6, th); // 散热鳍片
    ...
    _applyZ(z) { this.zCarriage.position.y = this.BED_Y + z; }
  ...
  /* ══════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════ */
  class FXPrinterDelta extends FXPrinterBase {
    _buildMachine() {
      ...
      /* 三塔柱（各自朝心旋转）+ 顶环 */
      const topY = 584, towerH = 520;
      this.towers = [];
      const tops = [];
      for (let i = 0; i < 3; i++) {
        const a = FXU.deg2rad(30 + i * 120);
        const tx = Math.cos(a) * this.TOWER_R, tz = Math.sin(a) * this.TOWER_R;
      ...
      for (let i = 0; i < 3; i++) {
        const s = this._beamBetween(
          V3().lerpVectors(tops[i], tops[(i + 1) % 3], 0.14).setY(topY - 16),
          V3().lerpVectors(tops[i], tops[(i + 1) % 3], 0.86).setY(topY - 16),
          3, null, stripMat);
        this.strips.push(s);
      }
    ...
    _beamBetween(a, b, sect, mat, overrideMat) {
    ...
    _updateKinematics() {
      ...
      const E_R = 40, C_OFF = 35, L = this.ARM_L;
      const ex = this.toolhead.position.x, ez = this.toolhead.position.z;
      ...
      for (let i = 0; i < 3; i++) {
        ...
        carY = FXU.clamp(carY, 100, 566);
        T.car.position.y = carY;
        // 双杆（沿切向 ±12mm 平行布置）
        const t = { x: T.u.z, z: -T.u.x };
        for (let s = 0; s < 2; s++) {
          const k = s === 0 ? 12 : -12;
          const ax = cj.x + t.x * k, az = cj.z + t.z * k;
          const bx = ej.x + t.x * k, bz = ej.z + t.z * k;
  ...
  /* ══════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════ */
  class FXPrinterGantry extends FXPrinterBase {
    _buildMachine() {
      ...
      for (const sx of [-1, 1]) for (const sz of [-1, 1])
        this._box(44, 18, 44, M.blackPlastic, sx * 252, -8, sz * 232);
      // 前缘警示橙条
      ...
      const stripMat = new THREE.MeshBasicMaterial({ color: 0xf2eee4 });
      this.strips = [
        this._box(400, 2.5, 5, stripMat, 0, 512, -214),
        this._box(400, 2.5, 5, stripMat, 0, 512, 214),
      ];
      ...
      shell.castShadow = true;
      for (let i = 0; i < 8; i++)
        this._box(44, 2, 22, M.aluBright, 0, -26 - i * 3.2, 14, th);   // 散热鳍片
    ...
    _applyZ(z) { this.zGantry.position.y = this.BED_Y + 52 + z; }
  ...
  root.FXPrinterI3 = FXPrinterI3;
  root.FXPrinterDelta = FXPrinterDelta;
  root.FXPrinterGantry = FXPrinterGantry;
