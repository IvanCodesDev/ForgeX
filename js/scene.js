...
(function (root) {
  ...
  class FXScene {
    constructor(canvas) {
      ...
      // vendor 若为 r152~r154（useLegacyLights 默认 true 的旧版），legacy 衰减会把
      // 900mm 范围内全部打成纯白过曝，必须显式切回物理光照；r155+ 本身即默认 false。
      if (parseInt(THREE.REVISION, 10) < 155 && "useLegacyLights" in renderer) {
    ...
    _buildFloorAndBackdrop() {
      // 地面：深蓝灰哑光 + 工程网格纹理（图三蓝图质感）
      const tile = document.createElement("canvas");
      tile.width = 256; tile.height = 256;
      const c = tile.getContext("2d");
      c.fillStyle = "#2e3440"; c.fillRect(0, 0, 256, 256);
      // 细网格 4×4
      c.strokeStyle = "rgba(196,210,232,0.075)"; c.lineWidth = 1;
      for (let i = 0; i <= 256; i += 64) {
        c.beginPath(); c.moveTo(i + 0.5, 0); c.lineTo(i + 0.5, 256); c.stroke();
        c.beginPath(); c.moveTo(0, i + 0.5); c.lineTo(256, i + 0.5); c.stroke();
      }
      // 粗格边缘
      c.strokeStyle = "rgba(196,210,232,0.13)"; c.lineWidth = 1.6;
      ...
      padCv.width = 512; padCv.height = 512;
      ...
      // 外缘细环标线
      pc.strokeStyle = "rgba(196,210,232,0.14)"; pc.lineWidth = 2;
      pc.beginPath(); pc.arc(256, 256, 238, 0, Math.PI * 2); pc.stroke();
      ...
      const cTop = new THREE.Color(0x3a4150), cMid = new THREE.Color(0x2a2f3a), cBot = new THREE.Color(0x1d212a);
    ...
    setCameraPreset(mode, immediate) {
      this.camMode = mode;
