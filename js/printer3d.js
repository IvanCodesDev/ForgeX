...
(function (root) {
  ...
  class FXPrinterBase {
    constructor() {
      ...
      // 动态状态（由 sim 驱动）
      this.fanFrac = 0;              // 物件冷却风扇 0..1
      this.nozzleHotFrac = 0;
      this.bedHotFrac = 0;
      this.extrudeRate = 0;          // mm/s（驱动料盘转动与熔融辉光）
      this.headMX = 0; this.headMY = 0;

      this._mats = this._makeMaterials();
      this._z = 0;                   // 喷嘴-床面纵向间距（虚拟 Z）
    ...
    _makeMaterials() {
      const M = {};
      M.frameDark = new THREE.MeshStandardMaterial({ color: 0x232220, metalness: 0.86, roughness: 0.42 });
      M.frameMid = new THREE.MeshStandardMaterial({ color: 0x424140, metalness: 0.8, roughness: 0.34 });
      M.aluBright = new THREE.MeshStandardMaterial({ color: 0xa8a6a1, metalness: 0.92, roughness: 0.24 });
      M.railSteel = new THREE.MeshStandardMaterial({ color: 0xd5d3ce, metalness: 1.0, roughness: 0.16 });
      M.blackPlastic = new THREE.MeshStandardMaterial({ color: 0x161616, metalness: 0.25, roughness: 0.65 });
      M.belt = new THREE.MeshStandardMaterial({ color: 0x0e0e0e, metalness: 0.1, roughness: 0.85 });
      M.brass = new THREE.MeshStandardMaterial({ color: 0xb98f3e, metalness: 1.0, roughness: 0.32 });
      M.copper = new THREE.MeshStandardMaterial({ color: 0x8a5a34, metalness: 1.0, roughness: 0.4 });
      M.acrylic = new THREE.MeshPhysicalMaterial({
        color: 0xb5b7b9, transparent: true, opacity: 0.09, roughness: 0.08, metalness: 0,
    ...
    _cyl(rt, rb, h, mat, x, y, z, parent, seg) {
    ...
    /* ── 共享部件：品牌屏（程序化纹理） ────── */
    _buildScreen(x, y, z, ry) {
      const cv = document.createElement("canvas");
      cv.width = 256; cv.height = 128;
      const c = cv.getContext("2d");
      c.fillStyle = "#10131a"; c.fillRect(0, 0, 256, 128);
      c.fillStyle = "#eef1f6"; c.font = "700 26px Consolas"; c.fillText(this.MODEL_TAG, 18, 44);
      c.fillStyle = "#2b323f"; c.fillRect(18, 62, 220, 8);
      c.fillStyle = "#ff6a2b"; c.fillRect(18, 62, 96, 8);
      c.fillStyle = "#7c8698"; c.font = "13px Consolas"; c.fillText(this.KIN_TAG + " · READY", 18, 96);
    ...
    _buildSpool(x, y, z, parent) {
      ...
      const flangeMat = new THREE.MeshStandardMaterial({ color: 0x151514, metalness: 0.3, roughness: 0.5, transparent: true, opacity: 0.85 });
      const f1 = new THREE.Mesh(new THREE.CylinderGeometry(52, 52, 3, 36), flangeMat);
      f1.position.y = 20; sp.add(f1);
      const f2 = f1.clone(); f2.position.y = -20; sp.add(f2);
    ...
    _buildBedSurface(g, opts) {
      ...
      grad.addColorStop(0, "#1e1f22"); grad.addColorStop(0.5, "#25272a"); grad.addColorStop(1, "#1c1d20");
      c.fillStyle = grad; c.fillRect(0, 0, 512, 512);
      const rnd = FXU.mulberry32(7);
      ...
      if (opts.round) {
        c.strokeStyle = "rgba(150,155,162,0.16)"; c.lineWidth = 2;
        c.beginPath(); c.arc(256, 256, 244, 0, Math.PI * 2); c.stroke();
        c.strokeStyle = "rgba(150,155,162,0.10)";
        c.beginPath(); c.arc(256, 256, 130, 0, Math.PI * 2); c.stroke();
        ...
        c.lineWidth = 2; c.strokeRect(10, 10, 492, 492);
        c.fillStyle = "rgba(170,174,180,0.28)";
        c.font = "600 17px Bahnschrift, Consolas";
    ...
    get bedTopY() { return this.NOZZLE_Y - this._z; }

    /** 床面真实世界高度：热床下沉机型动态、固定热床机型恒定，统一取 bedGroup 高度 */
    getBedWorldY() { return this.bedGroup.position.y; }
    ...
    setStateLED(hex) {
      this.stateLED.material.color.setHex(hex);
      this.powerLED.material.color.setHex(hex);
    }

    /** 料盘剩余比例（半径视觉反馈） */
    setSpoolFrac(f) {
      const r = 20 + FXU.clamp(f, 0, 1) * 26;
      this.filWound.scale.set(r / 46, 1, r / 46);
    }
    setFilamentColor(hex) {
      this.filWound.material.color.setHex(hex);
    }

    /* ══ 打印件 ═════════════════════════════ */

    attachPart(model, tf, colorHex, layerH) {
      this.clearPart();
      this.hideSlicePreview();     // 重切片后旧路径预览过期，由 UI 按需重建
      const geo = FXModels.buildGeometry(model);
      if (!geo) return;
      const uLayerH = { value: layerH / (tf.scale || 1) };
      const mat = new THREE.MeshStandardMaterial({
        color: colorHex, roughness: 0.5, metalness: 0.06, side: THREE.DoubleSide,
        clippingPlanes: [this.clipPlane], clipShadows: true,
      });
      mat.onBeforeCompile = (sh) => {
        sh.uniforms.uLayerH = uLayerH;
      ...
      const gmat = new THREE.MeshBasicMaterial({
        color: 0xff6a2b, transparent: true, opacity: 0.12, depthWrite: false,
      });
    ...
    showGhost(v) { if (this._ghost) this._ghost.visible = v; }
    showPart(v) { if (this._part) this._part.visible = v; }
    ...
    beginLayer(paths, zTop, layerH, colors) {
      this.endLayer();
      const g = new THREE.Group();
      ...
      for (const p of paths) {
        if (p.type !== runType) { flush(); runType = p.type; }
        run.push(p);
      }
      flush();
    }

    _buildRun(paths, type, radius, colors, parent) {
      const posArr = [], normArr = [], idxArr = [];
      let vOff = 0;
      const entries = [];
      for (const p of paths) {
        // 无法成管的路径也要占位（len 参与累计），否则揭示进度与 sim 的挤出里程失去对齐
        const skip = () => entries.push({ path: p, idxStart: 0, idxCount: 0, len: p.len, mesh: null });
        if (p.pts.length < 2 || p.len < 0.4) { skip(); continue; }
        // 去除近重合点，避免零长曲线导致 Frenet 帧 NaN
        const raw = p.pts.map((q) => V3(q.x, 0, -q.y));
        const pts3 = [raw[0]];
        for (let i = 1; i < raw.length; i++)
          if (raw[i].distanceTo(pts3[pts3.length - 1]) > 0.04) pts3.push(raw[i]);
        if (pts3.length < 2) { skip(); continue; }
        const curve = new THREE.CurvePath();
        for (let i = 1; i < pts3.length; i++)
          curve.add(new THREE.LineCurve3(pts3[i - 1], pts3[i]));
        const segs = FXU.clamp(Math.round(p.len / 1.6), Math.max(2, pts3.length - 1), 900);
        ...
        entries.push({ path: p, idxStart, idxCount: ti.length, len: p.len, mesh: null });
        tg.dispose();
      }
      ...
      if (posArr.length) {
        ...
        if (type === "support") { mat.color.setHex(colors.support); mat.transparent = true; mat.opacity = 0.85; }
        const mesh = new THREE.Mesh(geo, mat);
    ...
    /** layer: FXSlicer 层对象 { z, paths }；colors: { perimeter/solid/infill/support/skirt: hex } */
    showSlicePreview(layer, colors) {
    ...
    update(dt, elapsed) {
      ...
      // 喷嘴/热床辉光
      if (this.nozzle) this.nozzle.material.emissiveIntensity = this.nozzleHotFrac * 0.55;
      if (this.bedGlow) this.bedGlow.material.opacity = this.bedHotFrac * 0.16;
      ...
      if (this.chamberLights) {
        ...
        this.chamberLights.forEach((l, i) => { l.intensity = this._lightBase[i] * b; });
      }
      if (this._updateExtra) this._updateExtra(dt, elapsed);
      // 送料管重建（头部移动超过阈值时）
      this._tubeAcc += dt;
      if (this._tubeAcc > 0.09 && this.tubeMat) { this._tubeAcc = 0; this._rebuildTube(); }
    }
  ...
  /* ══════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════ */
  class FXPrinterCoreXY extends FXPrinterBase {
    _buildMachine() {
    ...
    /* 热床下沉：z 直接映射为床组高度 */
    _applyZ(z) { this.bedGroup.position.y = this.NOZZLE_Y - z; }
    ...
    _buildBase() {
      ...
      for (const sx of [-1, 1]) for (const sz of [-1, 1])
        this._cyl(16, 18, 10, M.blackPlastic, sx * 195, -3, sz * 195);
      // 前面板电控屏 + 电源灯
    ...
    _buildFrame() {
      const M = this._mats;
      const HX = 210, HZ = 210, Y0 = 69, Y1 = 496;
      const ext = 22; // 型材截面
      // 立柱
      for (const sx of [-1, 1]) for (const sz of [-1, 1]) {
    ...
    _buildPanels() {
      ...
      const h = Y1 - Y0 - 30, cy = (Y0 + Y1) / 2;
      const mk = (w, hh, x, y, z, ry) => {
    ...
    _buildZDrive() {
      const M = this._mats;
      this.screwPos = [[-150, -150], [150, -150], [0, 168]];
    ...
    _buildGantry() {
      ...
      // 前部惰轮
      for (const sx of [-1, 1])
        this._cyl(7, 7, 10, M.frameMid, sx * (HX - 34), GY + 15, HZ - 34);
    ...
    _buildToolhead() {
      ...
      // 散热鳍片
      for (let i = 0; i < 7; i++)
        this._box(34, 1.8, 20, M.aluBright, 0, -12 - i * 3.1, 16, th);
      // 热端风扇（正面，旋转）
      ...
      const nozzleMat = new THREE.MeshStandardMaterial({
        color: 0xb98f3e, metalness: 1.0, roughness: 0.3, emissive: 0xff5a1e, emissiveIntensity: 0,
      });
  ...
  root.FXPrinterBase = FXPrinterBase;
  root.FXPrinter = FXPrinterCoreXY;   // 兼容旧引用
  root.FXPrinters = FXPrinters;
