...
> **实施记录（2026-08-12 / Stage 6-A）**：已完成 C# G-code 作业韧性第一片。文件仓库持久化尝试次数、预算、下次执行和死信时间；确定性输入错误一次终止，瞬态读取/存储/超时/未知错误执行有界指数退避，预算耗尽进入稳定死信终态；Worker 重启会重新排队未完成作业。OpenAPI/TS Client、JobGate、队列深度/容量与 retry/recovery/dead-letter 指标、README/部署说明/Changelog 同步更新。租户配额、性能压测与容量计划、安全治理、正式 SLO/告警和恢复演练继续进入 Stage 6-B～6-D。

> **实施记录（2026-08-12 / Stage 6-B～6-D）**：生产强化闭环完成。文件仓库锁内执行幂等优先的 owner/tenant 活动作业准入，超限返回稳定 429；16 并发容量夹具、持久化状态/耗时/quota 指标和容量计划已固化。内部调用密钥支持当前值与一个前值的有界轮换，锁文件 registry、integrity、安装脚本、零 NuGet、容器镜像与密钥扫描进入 CI，并启用 Dependabot 周期审查。99.5% 可用性、99%/60 秒完成、失败率、RPO 0/RTO 5 分钟 SLO，六类 Prometheus 告警、逐项 Runbook、真实备份恢复演练与可切换镜像标签回滚均有机器证据；README、开发/部署说明与 Changelog 同步收口。单机文件仓库边界不宣称多副本能力，多副本仍以共享存储和排他领取为前置条件。
