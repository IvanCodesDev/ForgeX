...
> **Current release: v1.0.0.** The product boundary is explicit: offline file:// opening, Docker deployment, deterministic FDM simulation, G-code/Analytics authority, calibration governance, and production-data analysis are in scope; direct printer control, multi-tenant industrial platform features, real STL/3MF mesh import, multi-replica coordination, and unaudited energy/machine-time costing remain outside this release. Bundled data are conformance fixtures, not claims of factory accuracy.
...
The default rules engine completes statistical analysis without keys or external requests. Bring your own OpenAI-compatible endpoint — configure `OPENAI_*` on the server, or enter a Base URL / API key / model in the workbench AI settings (sent per request, never stored server-side) — and AI organizes the narrative while numbers, charts, and evidence remain generated by the local statistics pipeline.
...
└─ providers
   ├─ local rules
   └─ OpenAI-compatible endpoint (env-configured or user-supplied per request)
```
...
| Variable                                  | Purpose                                                                     |
| ----------------------------------------- | --------------------------------------------------------------------------- |
| `ANALYSIS_PROVIDER`                       | `auto`, `local`, or `openai`                                                |
